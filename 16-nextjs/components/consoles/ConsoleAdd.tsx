...
import {
    ...
    Image as ImageIcon, PlusCircle, UploadCloud, XCircle, Save
} from "lucide-react";

export default function ConsoleAdd() {
    // Nota: Aquí no necesitamos pasar 'consoles' como prop porque estamos creando una
    const [state, formAction, isPending] = useActionState(createConsole, null);
    ...
    useEffect(() => {
        if (state?.success) {
            toast.success("¡Consola registrada!", {
                description: "El nuevo hardware se añadió al catálogo.",
            });
        ...
        <div className="max-w-5xl mx-auto pt-4 pb-12 px-4 md:px-6">
            <form action={formAction} className="bg-base-300 p-6 md:p-10 rounded-[3rem] shadow-2xl border border-white/5 relative overflow-hidden">
                ...
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <h2 className="text-3xl font-black text-white tracking-tighter uppercase italic">Add New Console</h2>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    ...
                    <div className="space-y-5">
                        <div className="form-control">
                            ...
                            <input name="name" placeholder="E.g. PlayStation 5" className="input input-bordered bg-base-100/50 rounded-2xl focus:border-primary border-white/10 transition-all w-full" required />
                            {state?.fields?.name && <span className="text-error text-[10px] mt-1 italic">{state.fields.name}</span>}
                        </div>
                        ...
                        <div className="form-control">
                            ...
                            {state?.fields?.releasedate && <span className="text-error text-[10px] mt-1 italic">{state.fields.releasedate}</span>}
                        </div>
                    ...
                    <div className="flex flex-col gap-8">
                        <div className="form-control flex-1">
                            ...
                            <textarea
                                name="description"
                                placeholder="Describe the console features, specs or history..."
                                ...
                                required
                            ></textarea>
                            {state?.fields?.description && <span className="text-error text-[10px] mt-1 italic">{state.fields.description}</span>}
                        </div>

                        <div className="form-control">
                            <label className="label gap-2 justify-start text-primary font-bold uppercase text-[10px] tracking-widest">
                                <ImageIcon size={14} /> Console Image
                            </label>
                            <div className="relative group">
                                <div className="flex flex-col md:flex-row items-center gap-6 p-6 bg-base-100/30 rounded-4xl border-2 border-dashed border-white/10 group-hover:border-primary/50 transition-colors">
                                    {/* Preview Box - Adaptado a Consola (Más ancho) */}
                                    <div className="w-44 h-32 bg-base-300 rounded-2xl border border-white/5 flex items-center justify-center overflow-hidden shadow-2xl shrink-0 relative">
                                        {preview ? (
                                            ...
                                            <div className="text-center opacity-20">
                                                ...
                                                <span className="text-[8px] uppercase font-black">Hardware Pic</span>
                                            </div>
                                        )}
                                    </div>
